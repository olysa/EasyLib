...
import com.dingfang.org.example.okhttp.model.SimpleResponse;
import com.dingfang.org.example.okhttp.util.Convert;
    ...
    public T convertResponse(Response response) throws Throwable {
        ...
        if (type == null) {
            if (clazz == null) {
                ...
                type = ((ParameterizedType) genType).getActualTypeArguments()[0];//获得父类的泛型参数的实际类型
            } else {
                return parseClass(response, clazz);
            }
        }

        if (type instanceof ParameterizedType) {//使用这个LzyResponse泛型返回值基类都会走这条道
